...
- **Sync Tempo per person or team**: Tempo page now has team/person filter dropdowns — sync only selected people instead of everyone. Person Detail page has a "Sync Tempo" button for quick single-person sync. Much faster than full sync.
- **Actual hours on Timeline**: Each day cell now shows Tempo-logged hours (purple text below the allocation %). Summary column shows total actual hours for the period. Hover tooltip includes actual hours. Data comes from cached Tempo worklogs (no sync needed to view).
...
- **Jira issue key lookup**: Tempo API v4 only returns numeric issue IDs, not keys like "AMA-103". Now bulk-fetches issue keys from Jira (via JQL search, 100/batch) during sync/rematch to correctly map worklogs to projects.
